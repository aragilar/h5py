...
def generate_dcpl(shape, dtype, chunks, compression, compression_opts,
    ...
    def rq_tuple(tpl, name):
        ...
        try:
            ...
            raise TypeError(
                '"%s" argument must be None or a sequence object' %
                name)
        if len(tpl) != len(shape):
            raise ValueError(
                '"%s" must have same rank as dataset shape' %
                name)
    ...
    if compression is not None:
        ...
        if compression not in encode and not isinstance(compression, int):
            raise ValueError(
                'Compression filter "%s" is unavailable' %
                compression)
    ...
    if compression == 'gzip':
        ...
        if not h5z.filter_avail(compression):
            raise ValueError(
                "Unknown compression filter number: %s" %
                compression)
