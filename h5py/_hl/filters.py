...
def guess_chunk(shape, maxshape, typesize):
    ...
    if not np.all(np.isfinite(chunks)): # pylint: disable=no-member
        raise ValueError("Illegal value in chunk tuple")

    # Determine the optimal chunk size in bytes using a PyTables expression.
    # This is kept as a float.
    dset_size = np.product(chunks)*typesize
    target_size = CHUNK_BASE * (2**np.log10(dset_size/(1024.*1024))) # pylint: disable=no-member
    ...
    while True:
        ...
        chunks[idx%ndims] = np.ceil(chunks[idx%ndims] / 2.0) # pylint: disable=no-member
        idx += 1
